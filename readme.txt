...
* **Code that would have taken the site down is refused before it is saved.** A function that names the same parameter twice, or a closure that imports a variable it already receives, reads as correct to a syntax check and only fails when PHP runs it — as an error nothing can catch, on every page of the site. The check now recognises both. One hook on the list carried exactly that fault, so picking it was enough to trigger it.
* **The hook list was checked against what WPeMatico 2.9 actually fires.** Six hooks added during a campaign run in 2.9 are now offered — auto-category creation, the parent post of an item, which campaign types are fetched as a feed, the run summary line, and the two that shape a Vimeo item. Three hooks were being offered with fewer arguments than they are given, so the function written for them arrived incomplete, and six more built a function that PHP could not parse at all, which made them impossible to save. Every description was rewritten against the code it describes; the ones for audio, video and images said the same thing as each other and none of it was right. One entry named a filter that does not exist and now names the one that does.
* **Saved hooks now run on every request they should.** Adding `tab` to a web address was enough to make the plugin skip all of the saved code for that request, so the hooks could be turned off from outside the site, and they were also skipped on every other WPeMatico settings tab. Only this plugin's own screen is skipped now, which is what that check was there for.
