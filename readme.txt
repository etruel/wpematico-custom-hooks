...
* **A screen that belongs with the rest of WPeMatico.** The Hooks tab now carries its own icon in the menu, opens with a proper header, and sits inside WPeMatico's layout instead of pushing the page out of shape. The information about the plugin moved to WPeMatico's own sidebar, in this plugin's colours.
* **See at a glance what you are actually running.** The hook picker separates the ones you already have code on from the ones still available, and counts each group. Every box shows the hook's real name and is marked when it carries code, so you no longer have to open them one by one to find your own work.
* **Considerably quicker to open.** The code editor is now prepared for the hook you are looking at, rather than for all ninety of them on every visit. On a screen that used to build dozens of editors nobody was going to read, that is the difference between a page that waits and a page that is ready.
* **The hook list was brought in line with what WPeMatico does today**, including the additions that came with its latest release. Hooks that reached your function with fewer values than WPeMatico actually sends them are corrected, and every description was rewritten against WPeMatico's own code, so what the list tells you is what the hook does.
* **Nothing is stored that could bring the site down.** Some mistakes look perfectly correct to a syntax check and only fail once PHP runs them — at which point there is no catching them. Those are now recognised and refused before anything is saved, with the reason and the hook named.
